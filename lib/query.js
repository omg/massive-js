...
var operationsMap = {'=': '=', '!': '!=', '>': '>', '<': '<', '>=': '>=', '<=': '<=', '!=' : '<>', '<>': '<>'};
var nullOperationsMap = {'=': 'IS', '!': 'IS NOT', '!=': 'IS NOT'};
...
var Query = function(sql, params, table) {
  ...
  self.where = function(conditions) {
    ...
    _.each(conditions, function(value, key) {
      ...
      if (_.isNull(value)) {
        var operation = nullOperationsMap[parts[1]] || 'IS';
        return _conditions.push(util.format('"%s" %s NULL', property, operation));
      }
    ...
    return _conditions.length ? self._append(' \nWHERE ' + _conditions.join(' \nAND ')) : self;
  };
